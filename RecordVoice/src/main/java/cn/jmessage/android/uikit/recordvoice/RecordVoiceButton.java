package cn.jmessage.android.uikit.recordvoice;
...
public class RecordVoiceButton extends Button {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                    ...
                    Toast.makeText(this.getContext(), mContext.getString(IdHelper.getString(mContext, "jmui_sdcard_not_exist_toast")), Toast.LENGTH_SHORT).show();
    ...
    private void initDialogAndStartRecord() {
        ...
        if (myRecAudioFile == null) {
            cancelTimer();
            stopRecording();
            Toast.makeText(mContext, mContext.getString(IdHelper.getString(mContext, "jmui_create_file_failed")), Toast.LENGTH_SHORT).show();
        }
                    ...
                    if (!controller.mTimeUp) {
                        if (msg.what < CANCEL_RECORD) {
                            controller.mRecordHintTv.setText(controller.mContext
                                    .getString(IdHelper.getString(controller.mContext, "jmui_move_to_cancel_hint")));
                        }
                        else {
                            controller.mRecordHintTv.setText(controller.mContext
                                    .getString(IdHelper.getString(controller.mContext, "jmui_cancel_record_voice_hint")));
                        }
                        // 进入倒计时
                    } else {
                        if (msg.what == CANCEL_RECORD) {
